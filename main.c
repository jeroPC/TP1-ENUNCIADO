...
static void imprimir_pokemon(struct pokemon *pokemon) {
    if (!pokemon)
        return;
    printf("%d,%s,%d,%d,%d,%d\n", pokemon->id, pokemon->nombre ? pokemon->nombre : "",
           (int)pokemon->tipo, pokemon->ataque, pokemon->defensa, pokemon->velocidad);
}
...
int main(int argc, char *argv[]) {
    ...
    if (strcmp(comando, "buscar") == 0 && argc == 5) {
        ...
        if (pokemon)
            ...
            puts("No encontrado");
    } else if ((strcmp(comando, "union") == 0 || strcmp(comando, "interseccion") == 0 ||
        ...
        if (!otra_coleccion) {
            ...
            tp1_t *resultado_coleccion = operacion(pokedex, otra_coleccion);
            if (!resultado_coleccion || !tp1_guardar_archivo(resultado_coleccion, argv[4])) {
                fprintf(stderr, "Error generando/guardando resultado\n");
                resultado = 1;
            }
