...
enum tipo_pokemon string_a_tipo(const char *tipo_str) {
    ...
    else if (strcmp(tipo_str, "FUEG") == 0) return TIPO_FUEG;
    else if (strcmp(tipo_str, "PLAN") == 0) return TIPO_PLAN;
    else if (strcmp(tipo_str, "AGUA") == 0) return TIPO_AGUA;
    else if (strcmp(tipo_str, "NORM") == 0) return TIPO_NORM;
    else if (strcmp(tipo_str, "FANT") == 0) return TIPO_FANT;
    else if (strcmp(tipo_str, "PSI")  == 0) return TIPO_PSI;
    else if (strcmp(tipo_str, "LUCH") == 0) return TIPO_LUCH;
    errno = EINVAL;
    return TIPO_NORM;
}
 ...
 static char *leer_linea(FILE *archivo) {
    ...
    while (!terminada && fgets(buffer, LARGO_BUFFER, archivo)) {
        ...
        if (capacidad < requerido) {
            ...
            while (nueva < requerido) nueva *= 2;
            char *aux = realloc(linea, nueva);
            if (!aux) { free(linea); return NULL; }
            linea = aux;
            capacidad = nueva;
        }
    ...
    if (usados == 0) { free(linea); return NULL; }
    linea[usados] = '\0';
    if (usados > 0 && linea[usados - 1] == '\n') linea[usados - 1] = '\0';
    if (ferror(archivo)) { free(linea); return NULL; }
    return linea;
}
...
static struct pokemon* parsear_pokemon(const char *linea) {
    ...
    for (; i < 5; i++) {
        ...
        *coma = '\0';
        if (*campos[i] == '\0') { free(copia); return NULL; }
        p = coma + 1;
    }
    campos[5] = p;
    if (*p == '\0' || strchr(p, ',')) { free(copia); return NULL; }
...
tp1_t *tp1_leer_archivo(const char *nombre) {
    ...
    if (!tp1) { fclose(archivo); return NULL; }

    tp1->cantidad = 0;
    tp1->capacidad = 10;
    tp1->pokemones = malloc(sizeof(struct pokemon) * tp1->capacidad);
    if (!tp1->pokemones) { free(tp1); fclose(archivo); return NULL; }
    ...
    while ((linea = leer_linea(archivo)) != NULL) {
        ...
        if (p) {
            ...
            if (!repetido) {
                if (tp1->cantidad == tp1->capacidad && !redimensionar_Array(tp1)) {
                    free(p->nombre); free(p);
                    for (size_t i = 0; i < tp1->cantidad; i++) free(tp1->pokemones[i].nombre);
                    free(tp1->pokemones); free(tp1); fclose(archivo);
                    return NULL;
                }
                tp1->pokemones[tp1->cantidad] = *p;
                tp1->pokemones[tp1->cantidad].nombre = strdup(p->nombre);
                tp1->cantidad++;
            }
            free(p->nombre); free(p);
        }
...
static bool escribir_pokemon(FILE *archivo , struct pokemon *p){
    ...
    int len = snprintf(buffer, sizeof(buffer), "%d,%s,%s,%d,%d,%d\n",
    p->id,
    p->nombre,
    tipo_a_string(p->tipo),
    p->ataque,
    p->defensa,
    p->velocidad);
    
    if (len < 0 || len >= (int)sizeof(buffer)) return false;
    if (fputs(buffer, archivo) == EOF) return false;
    return true;
}
...
static tp1_t* combinar_tp1(tp1_t *a, tp1_t *b, criterio_pokemon_t criterio) {
    ...
    while (i < a->cantidad || j < b->cantidad) {
        struct pokemon *pa = (i < a->cantidad) ? &a->pokemones[i] : NULL;
        struct pokemon *pb = (j < b->cantidad) ? &b->pokemones[j] : NULL;
