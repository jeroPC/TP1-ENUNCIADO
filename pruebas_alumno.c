...
void tp1_leer_archivo_valido_devuelve_estructura()
{
	...
	pa2m_afirmar(tp1 != NULL,
		     "Abrir un archivo válido devuelve una estructura");
	if (tp1)
		tp1_destruir(tp1);
}
...
void buscar_nombre_devuelve_puntero_si_existe()
{
	...
	pa2m_afirmar(noexiste == NULL,
		     "Buscar por nombre devuelve NULL si no existe");
...
void diferencia_devuelve_tp1_con_pokemones_unicos()
{
	...
	pa2m_afirmar(tp1_cantidad(dif) == 1,
		     "Diferencia contiene solo los pokemones únicos de A");
...
void buscar_id_devuelve_puntero_si_existe()
{
	...
	pa2m_afirmar(p0 != NULL && strcmp(p0->nombre, "yyy") == 0,
		     "Buscar por ID 0 devuelve yyy");
	...
	pa2m_afirmar(p2 != NULL && (strcmp(p2->nombre, "Charmander") == 0 ||
				    strcmp(p2->nombre, "Charmand") == 0),
	...
	pa2m_afirmar(noexiste == NULL,
		     "Buscar por ID inexistente devuelve NULL");
...
static void prueba_guardar_con_nombre_muy_largo()
{
	...
	pa2m_afirmar(archivo != NULL,
		     "Se crea archivo temporal con nombre largo");
	tp1_t *tp = tp1_leer_archivo(archivo);
	pa2m_afirmar(tp && tp1_cantidad(tp) == 1,
		     "Se lee pokemon con nombre largo correctamente");
	...
	pa2m_afirmar(releido && tp1_cantidad(releido) == 1,
		     "Releer mantiene cantidad");
...
static void prueba_varios_nombres_largos()
{
	...
	char buffer[8192] = { 0 };
	for (size_t idx = 0; idx < 5u;
	     idx++) { /* idx como size_t para evitar sign-conversion */
		char nombre[600];
	...
	pa2m_afirmar(archivo != NULL,
		     "Se crea archivo temporal con varios nombres largos");
	tp1_t *tp = tp1_leer_archivo(archivo);
	pa2m_afirmar(tp && tp1_cantidad(tp) == 5,
		     "Se leen todos los pokemones de nombres largos");
	...
	pa2m_afirmar(p_test != NULL,
		     "Se puede buscar un pokemon intermedio tras guardado");
...
void test_tp1_con_cada_pokemon_recorre_todos()
{
	...
	size_t contador = 0;
	size_t recorridos =
		tp1_con_cada_pokemon(tp1, contar_pokemon, &contador);
	pa2m_afirmar(recorridos == tp1_cantidad(tp1),
		     "Recorre todos los pokemones");
	pa2m_afirmar(contador == tp1_cantidad(tp1),
		     "Callback se llama por cada pokemon");
	tp1_destruir(tp1);
}
...
void test_tp1_con_cada_pokemon_corta_antes()
{
	...
	pa2m_afirmar(recorridos == 2,
		     "Corta el recorrido después de dos pokemones");
...
static void prueba_lineas_invalidas_y_largas()
{
	...
	pa2m_afirmar(p5 && strcmp(p5->nombre, "Jigglypuff") == 0,
		     "Última línea válida se carga");
